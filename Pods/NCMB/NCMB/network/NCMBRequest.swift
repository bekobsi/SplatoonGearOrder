...
struct NCMBRequest {
    ...
    func getURL() throws -> URL {
        ...
        if let domainURL = URL(string: self.domainURL) {
            url = domainURL
        } else {
            throw NCMBInvalidRequestError.invalidDomainName
        }
        url.appendPathComponent(apiVersion, isDirectory: true)
        url.appendPathComponent(apiType.rawValue, isDirectory: false)
    ...
    private func getSortedQueryItems() -> [URLQueryItem] {
        ...
        for element in queryItems.sorted(by: { a, b in a.key < b.key }) {
