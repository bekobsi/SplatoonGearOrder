...
public class NCMBUser: NCMBBase {
    ...
    /// 現在ログインユーザー
    static var _currentUser: NCMBUser?
    ...
    override var copy: NCMBUser {
        let user = NCMBUser(className: className, fields: fields, modifiedFieldKeys: modifiedFieldKeys)
        return user
    }
    ...
    public class var currentUser: NCMBUser? {
        ...
        if let user = NCMBUser.loadFromFile() {
            _currentUser = user
            return user
        }
        return nil
    ...
    public func saveInBackground(callback: @escaping NCMBHandler<Void>) {
        ...
        let tmpuser = removeSessionToken()
    ...
    {
        let facebookInfo = NSMutableDictionary()
    ...
    public func signUpWithAppleToken(appleParameters: NCMBAppleParameters, callback: @escaping NCMBHandler<Void>) {
        let appleInfo = NSMutableDictionary()
    ...
    /// - Parameter callback: レスポンス取得後に実行されるコールバックです。
    public func signUpWithToken(snsInfo: NSMutableDictionary, callback: @escaping NCMBHandler<Void>) {
        let localAuthData = authData
        authData = snsInfo as? [String: Any]
        if localAuthData != nil {
            let localAuthDataDictionary = NSMutableDictionary(dictionary: localAuthData!)
            localAuthDataDictionary.addEntries(from: authData!)
            authData = localAuthDataDictionary as? [String: Any]
        }
        signUpInBackground(callback: { result in
            switch result {
            case .success:
    ...
    /// - Parameter callback: レスポンス取得後に実行されるコールバックです。
    public func linkWithToken(snsInfo: NSMutableDictionary, callback: @escaping NCMBHandler<Void>) {
        var localAuthData: [String: Any]?
        localAuthData = authData

        authData = snsInfo as? [String: Any]
        saveInBackground { result in
            switch result {
            case .success:
                // processing when saving is successful
                // Merge user object
    ...
    public func unlink(type: String, callback: @escaping NCMBHandler<Void>) {
        if authData != nil {
            if isLinkedWith(type: type) {
                let localAuthData = authData
                let localAuthDataDictionary = NSMutableDictionary(dictionary: authData!)
                ...
                let error = NSError(domain: "NCMBErrorDomain", code: 404_003, userInfo: [NSLocalizedDescriptionKey: "other token type"])
                callback(NCMBResult<Void>.failure(error))
            }
        } else {
            let error = NSError(domain: "NCMBErrorDomain", code: 404_003, userInfo: [NSLocalizedDescriptionKey: "token not found"])
            callback(NCMBResult<Void>.failure(error))
        }
    }
}
